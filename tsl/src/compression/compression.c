...
tsl_compressor_flush(RowCompressor *compressor, BulkWriter *bulk_writer)
{
	if (compressor->sort_state)
	{
		if (compressor->tuples_to_sort)
		{
			...
			{
				row_compressor_flush(compressor, bulk_writer, /* changed_groups = */ false);
			}
...
row_compressor_flush(RowCompressor *row_compressor, BulkWriter *writer, bool changed_groups)
{
	...
	if (row_compressor->invalidation.hypertable_id != 0)
	{
		...
		/* tsl_compressor_add_slot recorded each input row's tenant on the way in,
		 * so this range has full tenant coverage. */
		continuous_agg_invalidate_range(settings->hypertable_id,
										settings->chunk_relid,
										start,
										end,
										false /* tenants_unknown */);
	}
