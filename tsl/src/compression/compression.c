...
row_decompressor_close(RowDecompressor *decompressor)
{
	...
	if (decompressor->decompressed_slots)
	{
...
decompress_batch(RowDecompressor *decompressor)
{
	...
	/* ALWAYS LOG: Debug trace to see if our decompression code is called */
	elog(DEBUG1, "decompress_batch called: n_batch_rows=%d, GUC enable_virtual_slots_decompression=%s",
		 n_batch_rows, ts_guc_enable_virtual_slots_decompression ? "true" : "false");
    ...
    if (n_batch_rows > decompressor->max_slots)
    {
        ...
        elog(DEBUG2, "Expanded decompressed slots array from %d to %d",
             old_max, new_size);
    }
	...
	for (int current_row = 0; current_row < n_batch_rows; current_row++)
	{
		...
		if (ts_guc_enable_virtual_slots_decompression)
		{
			 ...
			 */
			elog(DEBUG1, "SUCCESS: Using virtual slots for row %d (GUC is enabled)", current_row);
		...
		else
		{
			...
			elog(DEBUG1, "WARNING: Using heap_form_tuple for row %d (GUC is disabled)", current_row);
