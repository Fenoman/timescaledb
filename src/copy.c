		 ...
		 void (*callback)(void *), void *arg)
{
	...
	for (;;)
	{
		...
		/* Preserve original hypertable tuple for statement-level NEW TABLE transition tables */
		if (ccstate->cstate && ccstate->cstate->transition_capture)
		{
			...
			bool chunk_has_before_trig = (chunk_trigdesc && chunk_trigdesc->trig_insert_before_row);
